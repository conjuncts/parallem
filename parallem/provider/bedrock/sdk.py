...
def _prepare_bedrock_body(
    ...
    model_kwargs: dict,
) -> tuple[Union[str, bytes, dict], dict]:
    ...
    :return: A tuple of (body, invoke_options).
    """
    ...
    if bedrock_body is not None:
        if isinstance(bedrock_body, dict) and model_kwargs:
            return {**bedrock_body, **model_kwargs}, invoke_options
        if model_kwargs:
            raise ProviderCompatibilityError(
                "bedrock_body is not a dict; cannot merge model parameters."
            )
        return bedrock_body, invoke_options
...
class SyncBedrockProvider(SyncProvider, BedrockProvider):
    ...
    def prepare_sync_call(
        ...
        **kwargs,
        ...
        body, invoke_options = _prepare_bedrock_body(params, model_kwargs)
